...
<?php
	...
	{
		if(isset($_POST['transfer'])) //make sure to make it check if the worker being removed is not scheduled for an upcoming work order
		{
			$curdate=date("Y-m-d");
			$curdate=date("Y-m-d", strtotime($curdate.'+8days'));
			...
			//not allowing the removal of an employee if the have a work item scheduled
			//prevents a manager who have people under them from being removed
			...
			$remove=$link->prepare("DELETE wi_schedule, workitem
									FROM workitem
									INNER JOIN wi_schedule on wi_schedule.itemid=workitem.itemid
									WHERE employeeID=?");
			$remove->bind_param("i",$holder);
			$remove->execute();
            ...
            <div class="modal-body modalContent">
              <form class = "form1" action = "AdminLanding.php" method = "POST">
                <div class="form-group">
                    ...
                    <input type="Text" class="form-control" name="fname" placeholder="First Name" maxlength="15" required>
                </div>   
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" name="em" placeholder="example@pepsico.org" maxlength="50" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" pattern="(?=^.{6,}$)((?=.*\d)(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" title="Must be at least 6 characters long and contain at least 1 number, 1 uppercase letter, 1 lowercase letter, and 1 special character" name="pword" placeholder="Password" type="password" required> <!-- we need to end up hashing this-->
			
                </div>   
                <div class="form-group">
                    <label>Confirm Password:</label>
                    <input class="form-control" pattern="(?=^.{6,}$)((?=.*\d)(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" title="Must be at least 6 characters long and contain at least 1 number, 1 uppercase letter, 1 lowercase letter, and 1 special character" name="pwordc" placeholder="Password" type="password" required> <!-- we need to end up hashing this-->
