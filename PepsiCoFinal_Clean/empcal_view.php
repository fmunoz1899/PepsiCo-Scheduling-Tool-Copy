    ...
    <body>
          ...
          <ul class="navbar-nav">	
            ...
            <div class="jumbotron text-center jumbotron2">
                <h1 class="font-weight-bold text-center">Engineer Workorder Calendar View</h1>
<?php 
	include('connect.php');
  ...
  <div class="modal-dialog">
	...
	<h5 class="modal-title" >Workorder Details</h5>
		...
		<div class="modal-body modalContent">
		 <form class = "form1" id = "CalUpdateTime" action = "empcal_view.php" method = "POST">
...
<?php
		$today=date("Y-m-d");//to know what the actual date is to be able to properly label buttons
...
echo"
 ...
 echo"	            <tr class = 'row_height'>";
						...
						if($first)
						{
							if(($curtime>=$row['endtime'] && $today==$curdate) || $today>$row['date']) 
							{
								if(strtotime('+15 minutes',strtotime($row[0]))==strtotime($row[1]))
								{
									...
									<button class='btn btn-primary EditWOCalEmp' type='button' data-toggle='modal' data-target='#EmpWOCAl' value='" . $row[3] . "'>COMPLETED</button>
							...
							else
							{
								if(strtotime('+15 minutes',strtotime($row[0]))==strtotime($row[1]))
								{
									$WOCalID = $row[3];
									echo "<td class='sched_work_15_min'>
									<button class='btn btn-success EmpWOCAlPassed' type='button' data-toggle='modal' data-target='#EmpPassedWO' value='" . $row[3] . "'>Details</button>
									<table style = 'display:none'>
											<tr>
												<td id = ".$WOCalID."name>".$rowsch["firstName"]. " " . $rowsch["lastName"] . "</td>
												<td id = ".$WOCalID."pid>".$rowsch['employeeID']."</td>";
					...
					if($time>=$rowblock[0] && $time<$rowblock[1] && !$entered) //to show when an employee has a blockout time 
					{
						if($first2)
						{
							if(($curtime>=$rowblock[1] && $today==$curdate) || $today>$curdate)
							{ 
								$BID =  $rowblock[2];
							...
							else
							{
								...
								if(strtotime('+15 minutes',strtotime($rowblock[0]))==strtotime($rowblock[1]))
								{
									...
									echo "<td class='blockout_15_min'>
									<button class='btn btn-success BOCalDet' type='button' data-toggle='modal' data-target='#DetBO'  value='".$rowblock[2]."'>Details</button>
									<table style = 'display:none'>
									<tr>
										<td id = ".$BID."name>".$rowsch["firstName"]. " " . $rowsch["lastName"] . "</td>
										<td id = ".$BID."sTime>" . date('g:ia',strtotime($rowblock[0]))."</td>
