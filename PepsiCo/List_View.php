...
<head>
 ...
 <?php 
	...
	if(isset($_POST['orderID']))
	{
		...
		else if($_POST['datepicker5']==date("m/j/Y") && $startTime<=$curtime)
		...
		{
			//echo"<script>alert('".$_POST['e1e']."');</script>";
			//echo"<script>alert('".$curday."');</script>";
	...
	if(isset($_POST['sTimel']) && isset($_POST['eTimel']) && isset($_POST['loc']) && isset($_POST['e1']) && isset($_POST['meth']))
	{
		...
		else if($_POST['datepicker3']==date("m/j/Y") && $_POST['sTimel']<=$curtime)
			echo"<script>alert('You cannnot make it for the past');</script>";
          ...
          echo "<tr>
			...
			while($row = $result->fetch_assoc()) 
			{
				...
				if($row['actualendtime']=='')
					echo"<td><button class='btn btn-success EditWO' type='button' data-toggle='modal' data-target='#EdWO' value='" . $row['ItemID'] . "'>Edit</button>";
				else
