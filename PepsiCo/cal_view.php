				...
				$rowh = mysqli_fetch_array($hres);
					...
					else if($rowh[0]==0 && $rowb[0]==0 && $roww[0]==0)
					{
						...
						$upwo2=$link->prepare("UPDATE workitem
											   ...
											   preferrede2=?
											   WHERE itemid=?");
						$upwo2->bind_param("iiiii",$_POST['loce'],$_POST['methe'],$_POST['e2e'],$_POST['e3e'],$_POST['orderID']);
						$upwo2->execute();
					}
...
if(isset($_POST['EmpID']) && isset($_POST['BOtimeID']))
{
	...
	$reason=filter_var(htmlentities($_POST['reasonEditBO'],  ENT_QUOTES,  'utf-8'),FILTER_SANITIZE_STRING);
		...
		if($startTimeBO>=$endTimeBO)
			echo"<script>alert('You cannot start at or after the fffend time');</script>";
		
		else if($_POST['datepicker6']==date("m/j/Y") && $startTimeBO<$curtime)
			echo"<script>alert('You cannnot make it for the past');</script>";
