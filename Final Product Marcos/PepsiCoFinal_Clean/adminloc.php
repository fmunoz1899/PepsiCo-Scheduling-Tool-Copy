...
<?php
	...
	if(isset($_POST['id']) && $_POST['id']!='')
	{
		...
		$check=$link->prepare("SELECT locationID 
							   FROM workitem, wi_schedule
							   WHERE locationID=?
							   AND workitem.itemid=wi_schedule.itemid
							   AND date>=?");
			$check->bind_param("is",$_POST['id'],$curdate);
			$check->execute();
          ...
          <div class="modal-content">
			...
			<div class="col-md-8">
			  ...
			  <label> State:</label> <br>
                	<select name="state" id="stateAEdit" required>  
				  <option value="" selected>--Select One--</option>
				  <option value="AL">Alabama</option>
				  <option value="AK">Alaska</option>
				  <option value="AZ">Arizona</option>
    ...
    echo "<div class='col-md-1'> </div>";
        echo "<div class = 'col-md-10'>";
          ...
          while($row = $result->fetch_assoc()) 
          {
            ...
            echo "<tr>
			  <td style = 'width: 80px'><button class='btn btn-success ELocAdmn' type='button' data-toggle='modal' data-target='#EditLocAdm' value='" . $row['LocationID'] . "'>Edit</button></td>
			  <td style = 'width: 80px'><form method='POST' class = 'hiddenform' action='adminloc.php'><input name='id' type='hidden' value=".$row['LocationID']."><button class = 'btn btn-danger'>Remove</button></form></td>
