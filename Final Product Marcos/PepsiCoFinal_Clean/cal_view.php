...
if((isset($_POST['forall']) || isset($_POST['who'])) && isset($_POST['sTimec']) && isset($_POST['eTimec'])) //to creat block out either for all or 
{
	$curtime= date('H:i:s');
...
?>
			...
			<div class="form-group">
              <label>By Filtered Manager <input type="checkbox" id="forall" name="forall" checked></label>
            </div>
			<div class="form-group">
                <label>Who:</label>
...
include('connect.php');
			...
			<div class="form-group">
                <label>Reason</label>
                <textarea rows = "5" cols = "25" name="reason" maxlength="100" id="reason" placeholder="Meeting"></textarea>
				<span id="reasonsleft">100</span> Character(s) Remaining
            </div>
                <button type="submit" name="submit" class="btn btn-primary">Blockout</button></br></br> 
              </form>
            </div>
			</div>
            <div class="modal-footer">
 ...
 <!--------------- Modal Code To Edit -------------->
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
			...
			<form method='POST' action='cal_view.php' class = 'hiddenform form2'>
				<input name='CalWOID' type='hidden' id = "CalWOID" value=""><button class = 'btn btn-danger'>Remove</button>
			</form>
				<button type="button" class="btn btn-secondary form2" data-dismiss="modal">Close</button>
				...
				<div class = "col-md-9">
					...
					<div class="form-group">
						<label>Reason:</label><br>
						 <textarea rows = "5" cols = "25" maxlength="100" name="reasonEditBO" id="reasonEdit"></textarea>
	...
	<div class="modal fade" id="DetBO" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLongTitle">Blockout Details</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
											...
											<tr>
												...
												$name1=$link->prepare("SELECT firstname, lastname, employeeid
													...
													if($row2[0]!='' && $row2[1]!='')
													{
														...
														echo"<td id = ".$WOCalID."sid>".$row2['employeeid']."</td>"; 
													}
							...
							else
							{
								if(strtotime('+15 minutes',strtotime($row[0]))==strtotime($row[1]))
								{
									...
									$locname=$link->prepare("SELECT locationname
													...
													$rowl=mysqli_fetch_array($temp1);
									echo "<td class='sched_work_15_min'>
									<button class='btn btn-success EditWOCal' type='button' data-toggle='modal' data-target='#EdWOCal' value='" . $row[3] . "'>".$rowl[0]."</button>";
								...
								else
								{
									...
									$locname=$link->prepare("SELECT locationname
													...
													$temp1=$locname->get_result();
													$rowl=mysqli_fetch_array($temp1);
										echo"<button class='btn btn-success EditWOCal' type='button' data-toggle='modal' data-target='#EdWOCal' value='" . $row[3] . "'>".$rowl[0]."</button>";
										echo"<table style = 'display:none'>
											<tr>
												<td id = ".$WOCalID."name>".$rowsch["firstName"]. " " . $rowsch["lastName"] . "</td>
												<td id = ".$WOCalID."pid>".$rowsch['employeeID']."</td>";
