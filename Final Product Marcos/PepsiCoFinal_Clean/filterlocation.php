...
echo"   <div class='row tbl_space'> 
		...
		while($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
			  ...
			  <td style = 'width:80px'><button class='btn btn-success ELocFltrMgr' type='button' data-toggle='modal' data-target='#EditLocMgrFt' value='" . $row['LocationID'] . "'>Edit</button></td>
			  <td style = 'width:80px'><form class = 'hiddenform' method='POST' action='filterlocation.php'><input name='filterl' type='hidden' value=".filter_var(htmlentities($_POST['filterl'],  ENT_QUOTES,  'utf-8'),FILTER_SANITIZE_STRING)."><input name='id' type='hidden' value=".$row['LocationID']."><button class = 'btn btn-danger'>Remove</button></form></td>
