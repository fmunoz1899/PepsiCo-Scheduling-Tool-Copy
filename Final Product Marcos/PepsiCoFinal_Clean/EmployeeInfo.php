...
<?php
			...
			{
				$ID=filter_var($_POST['transfer2'], FILTER_SANITIZE_NUMBER_INT); //phone number 
				$ID2=filter_var($_POST['transferID'], FILTER_SANITIZE_NUMBER_INT);//empID
...
?>
				<form class = "hiddenform2" method="POST" action="changePass.php">
				<?php
...
?>
				...
				<br>
					<?php
					echo"<form class = 'hiddenform2' method='POST' action='AdminLanding.php'>";
...
echo"			<input type='hidden' value='".$clean."' name='transfer'> ";
				if(isset($_POST['manid2']))
					...
					$self=$link->prepare("SELECT employeeid
										  FROM email
										  WHERE email=?");
					$self->bind_param("s",$clean);
					$self->execute();
					$ans=$self->get_result();
					$rowself=$ans->fetch_assoc();
					
					if($rowself['employeeid']!=$_SESSION['username']) //to not allow the user to delete themself 
						echo"<button class='btn btn-primary' type='submit'>Remove Employee</button>";
				?>
            ...
            <div class="modal-body modalContent">
			  ...
			  <input type="hidden" name="transfer" value=<?php echo filter_var(htmlentities($_POST['transfer'],  ENT_QUOTES,  'utf-8'),FILTER_SANITIZE_EMAIL);?>><!-- this allows the primary email continue to be used for the specific employee-->
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" name="addemail" placeholder="example@pepsico.org" maxlength="50">
                </div>
                <div class="form-group">
		...
		<div class="modal-body modalContent">
			<form class = "form1" action = "EmployeeInfo.php" method = "POST">
				<input type="hidden" name="transfer" value=<?php echo filter_var(htmlentities($_POST['transfer'],  ENT_QUOTES,  'utf-8'),FILTER_SANITIZE_EMAIL);?>><!-- this allows the primary email continue to be used for the specific employee-->
					<div class="form-group">
						<label>Email (will be set as personal)</label>
						<input type="email" class="form-control" name="addemail" id = "EmailToEdit" maxlength="50">
					</div>
                ...
                <table>
					...
					while($emrow=$finalem->fetch_assoc())
					{
					   if($emrow['type']!='Work')
					   {
							...
							<form class = 'hiddenform' method='POST' action='EmployeeInfo.php'> 
