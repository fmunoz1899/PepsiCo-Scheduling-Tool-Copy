...
?>
    ...
    <body>
        ...
        <div class="jumbotron text-center jumbotron2">
            <h1 class="font-weight-bold text-center">Transfer Workorder</h1>
...
<?php
	...
	{
		header("location:login.php?trans_no_man_tok");
		exit;
	}
					...
					while($row=$ans->fetch_assoc())
					{
						...
						$checkhours=$link->prepare("SELECT CASE WHEN EXISTS
								...
								else if($rowh[0]==0 && $rowb[0]==0 && $roww[0]==0 && !($startdate==$today && $row['StartTime']<=$curtime))
								{
									...
									$rem->bind_param("i",$row['ItemID']);
									$rem->execute();*/
									
									
									//not sure if reoccurring should be carried over
									//penis
									if($_POST['tran2']==$row['PreferredE1'])
									{
										...
										//$worki->bind_param("iiisi",$_POST['tran2'],$row['LocationID'],$row['DeliveryID'],$row['Description'],$row['PreferredE2']);
									...
									else if($_POST['tran2']==$row['PreferredE2'])
									{
										...
										//$worki=$link->prepare("INSERT INTO workitem 
												//VALUES (default,?,?,?,?,default,?,default)");
										//$worki->bind_param("iiisi",$_POST['tran2'],$row['LocationID'],$row['DeliveryID'],$row['Description'],$row['PreferredE1']);
									...
									else
									{
										...
										//$worki->bind_param("iiisii",$_POST['tran2'],$row['LocationID'],$row['DeliveryID'],$row['Description'],$row['PreferredE1'],$row['PreferredE2']);
									}
									...
									//pussy
									//when transferred the reoccurring flag is also carried, not sure if that should be
									$please=$link->prepare("INSERT INTO wi_schedule
														   VALUES (default,?,default,?,?,?,?)");
									$please->bind_param("issss",$num,$row['StartTime'],$row['EndTime'],$startdate,$row['reoccurring']);
									$please->execute();*/
								}
					}
