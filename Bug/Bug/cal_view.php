...
if(isset($_POST['sTimel']) && isset($_POST['eTimel']) && isset($_POST['loc']) && isset($_POST['e1']) && isset($_POST['e2']) && isset($_POST['e3'])&& isset($_POST['meth']))
	{
		echo"<script>alert('".$_POST['meth']."');</script>";
...
<?php	/*			
					...
					while($row = $result->fetch_assoc()) 
						echo"<option value=".$row['deliveryid'].">".$row['method']."</option>";
						*/
?> 
                    </select>-->
              </div>
			...
			<form class = "form1" action = "cal_view.php" method = "POST" >
              <div class="form-group">
                <label> First Choice Engineer:</label>
                    <input type = "text" name="employee" id = "MainEng"  disabled>
			  ...
			  <div class="form-group">
                  ...
                  <input type="time" id = "aed" class="form-control" name="time" disabled>
              </div>
              <div class="form-group">
                  <label> Start Time:</label>
...
echo"
          <div class='item'>
            <table class = 'cal_tbl_bg'>
              <tr>
				...
				$blackout="SELECT starttime, endtime, blackoutID FROM blackout WHERE blackout.BDate='".$curdate."' and blackout.EmployeeID=".$rowID['employeeID']." order by StartTime";
				$result4=mysqli_query($link,$blackout);
				$rowblock = mysqli_fetch_array($result4);

				$BlackoutID = $rowblock['blackoutID'];
				...
				and employee.employeeid=".$rowID['employeeID']."
				and PRIVILEGEid='E'
				and Date=?
				ORDER BY Date, StartTime, EndTime");
				$sqlWI->bind_param("s",$curdate);
				$sqlWI->execute();
				$resq=$sqlWI->get_result();
 ...
 echo"	            <tr class = 'row_height'>";
							...
							else
							{
								...
								if ($first == 1)
								{
									echo "<td class='sched_work'><button class='btn btn-success sid' type='button' data-toggle='modal' data-target='#Details' value='" . $row['scheduleid'] . "' >Details</button>
									 ...
									 {
										echo"<input type = 'hidden' id =' ".$WorkorderID."pe' value='".$res["firstName"]. " " . $res["lastName"]."'>";
											...
											$name1=$link->prepare("SELECT firstname, lastname
																...
																$r2 = mysqli_fetch_array($temp1);
																
											echo"				<input type = 'hidden' id =' ".$WorkorderID."se' value = '".$r2[0]." ".$r2[1]."'>";
										...
										//echo"<input type='hidden'>".$r2[0]." ".$r2[1]."></input>";
																
										/*echo"<input type = 'hidden' id = ".$WorkorderID."te>".$r2[0]. " " . $r2[1] . ">
										<input type = 'hidden' id = '".$WorkorderID."dte'>" . $res["Date"].">
										<input type = 'hidden' id = '".$WorkorderID."aet'>" . $res["actualendtime"].">
										<input type = 'hidden' id = '".$WorkorderID."st'>" . $res["StartTime"].">
										<input type = 'hidden' id = '".$WorkorderID."et'>" . $res["EndTime"].">
										<input type = 'hidden' id = '".$WorkorderID."loc'>" . $res["LocationName"].">
										<input type = 'hidden' id = '".$WorkorderID."meth'>" . $res["Method"].">
										<input type = 'hidden' id = '".$WorkorderID."des'>" . $res["description"].">";*/
									 }
					...
					if(!$entered) 
					//if there is nothing happening at a specific time while the employee is on the clock
						echo "<td>	</td>";
						//$first=1;
