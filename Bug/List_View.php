...
<?php				
					...
					$result2=$link->query($people);

					while($rowm = $result2->fetch_assoc()) 
						echo"<option value=".$rowm['deliveryid'].">".$rowm['method']."</option>";
?> 
                    </select>
              </div>
              <div class="form-group">
...
//Make sure to change this so that it only shows the engineers
	...
	$sqlWI="SELECT firstName, lastName, workitem.ItemID, LocationName, Method, ActualEndTime, StartTime, EndTime, Date, preferrede1, preferrede2, description
